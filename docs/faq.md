...
## Does Solgo Insight use raw xDrip+ Local data?

Yes.

When xDrip+ Local is configured, Solgo Insight reads raw glucose readings from the xDrip+ local web service.

Solgo Insight does not import xDrip+'s precomputed stats or display values. It syncs glucose readings, stores them locally, and calculates its own Home, History, Stats, Insights, and chart views from those readings.

## Does Solgo Insight require Nightscout?

No.

Solgo Insight can use xDrip+ Local as a data source when it is available.
...
4. Open Solgo Insight.
5. Go to **Data Source** and connect **xDrip+ Local**.

## Does Solgo Insight import xDrip+ stats?

No.

Solgo Insight currently calculates its own stats from synced glucose readings.

This means Solgo Insight may not always match xDrip+'s internal stats exactly, especially if:

- The initial sync has not completed.
- The available history window is different.
- xDrip+ and Solgo Insight use different calculation windows.
...
Solgo Insight may show a rate per minute, for example:
...
Solgo Insight will continue improving the UI wording so users can clearly distinguish:
...
Solgo Insight calculates Stats and Insights from the readings that have been synced locally.
...
Solgo Insight includes multiple widget layouts and sizes.
...
## Does Solgo Insight replace xDrip+?

No.

Solgo Insight is a companion app for people who already use xDrip+ and Nightscout.
...
## Will Solgo Insight support Wear OS?

Wear OS support is not a current focus.

xDrip+ already has strong Wear OS support, including watch faces and complications. Solgo Insight's current focus is data review, analysis, insights, and Android glance surfaces such as widgets and persistent notification previews.

This may be revisited later, but the current roadmap is focused on Solgo Insight's differentiation as a data analysis and companion experience.
...
## Is Solgo Insight a medical device?

No.

Solgo Insight is not a medical device and should not be used as the only basis for treatment decisions.
