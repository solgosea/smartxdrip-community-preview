...
import 'package:smart_xdrip/plugins/datasource/alert_source/local_glucose_alert_source.dart';

import '../../_support/test_database.dart';

void main() {
  test('local datasource alerts stay silent when no datasource is enabled',
      () async {
  ...
  test('local datasource alerts follow Alerting Core rules', () async {
    ...
    await database.upsertMany([
      GlucoseReading(timestamp: now, value: 3.4),
    ]);
    final sink = _CollectingSink();
    final source = LocalGlucoseAlertSource(
      database: database,
