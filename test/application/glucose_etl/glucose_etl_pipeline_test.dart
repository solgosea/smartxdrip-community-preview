...
void main() {
  group('GlucoseEtlPipeline', () {
    test('uses Nightscout as the deterministic winner for history buckets',
        () async {
      ...
      await pipeline.run(
        source: 'nightscout',
        readings: [
          GlucoseReading(timestamp: sampleTime, value: 6.8),
        ],
        now: now,
      );
      await pipeline.run(
    ...
    test('uses xDrip local as the deterministic winner for recent buckets',
        () async {
      final database = TestDatabase.create();
      addTearDown(database.close);
      final now = DateTime(2026, 6, 5, 12);
