...
void main() {
  test('initializes self default alert rules in Alerting Core tables',
      () async {
    ...
    expect(
        rules.map((rule) => rule.category), contains(AlertCategory.glucoseLow));
    ...
    expect(
        urgentLow.thresholdValue, GlucoseAlertDefaultThresholds.urgentLowMmol);
    ...
    expect(rapidFall.thresholdValue,
        GlucoseAlertDefaultThresholds.rapidFallRateMmolPerMin);
    expect(noData.thresholdValue, GlucoseAlertDefaultThresholds.noDataMinutes);
    expect(
