...
class HighEpisodeBaselineCalculator {
  const HighEpisodeBaselineCalculator();

  HighEpisodeBaselineContext calculate({
    required DateTime episodeTime,
    required EpisodeWindowAnalysis window,
    required List<GlucoseReading> allReadings,
  }) {
    final peaks = _dailyPeaks(allReadings, episodeTime, 30)..sort();
    final variability = _variabilityForHour(allReadings, episodeTime.hour);
    final slopeWindow = _periodForHour(episodeTime.hour);
    return HighEpisodeBaselineContext(
      baselineLowMmol: window.baselineLow,
      baselineHighMmol: window.baselineHigh,
      usualPeakLowMmol: _percentile(peaks, 25),
      usualPeakHighMmol: _percentile(peaks, 75),
      ...
      typicalSlope: _typicalSlopeForHours(
        allReadings,
        startHour: slopeWindow.start,
        endHour: slopeWindow.end,
      ),
    );
  }

  List<double> _dailyPeaks(
    ...
    return [
      for (final rows in byDay.values)
        rows.map((row) => row.value).reduce(math.max),
    ];
  }
  ...
  PeriodVariability? _variabilityForHour(
    List<GlucoseReading> readings,
    int hour,
  ) {
    final periods = _periodBuckets;
  ...
  _PeriodBucket _periodForHour(int hour) {
    return _periodBuckets.firstWhere(
      (p) => _isInsideHourWindow(hour, p.start, p.end),
      orElse: () => _periodBuckets.first,
    );
  }
...
const _periodBuckets = <_PeriodBucket>[
  _PeriodBucket('Overnight', '00:00-06:00', 0, 6),
  _PeriodBucket('Morning', '06:00-12:00', 6, 12),
  _PeriodBucket('Afternoon', '12:00-18:00', 12, 18),
  _PeriodBucket('Evening', '18:00-24:00', 18, 24),
];
