...
class EpisodeWindowAnalysis {
  ...
  final List<GlucoseReading> episodeReadings;
...
class EpisodeWindowCalculator {
  const EpisodeWindowCalculator();

  EpisodeWindowAnalysis calculate({
    ...
    final episodeEnd = event.endTime ??
        event.time.add(Duration(minutes: math.max(event.durationMinutes, 0)));
    final end = episodeEnd.add(const Duration(hours: 2));
    final readings = allReadings
        .where((r) => !r.timestamp.isBefore(start) && !r.timestamp.isAfter(end))
        .toList();
    final episodeReadings = readings
        .where((r) =>
            !r.timestamp.isBefore(event.time) &&
            !r.timestamp.isAfter(episodeEnd))
        .toList();
    ...
    final leadUp = readings
        .where((r) =>
            ...
            r.timestamp.isBefore(event.time))
        .toList();
    final extreme = episodeReadings.isEmpty
        ? null
        : episodeReadings.reduce((a, b) =>
    ...
    return EpisodeWindowAnalysis(
      ...
      readings: readings,
      episodeReadings: episodeReadings,
